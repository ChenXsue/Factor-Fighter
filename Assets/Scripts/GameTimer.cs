...
public class GameTimer : MonoBehaviour
{
    ...
    private GoogleSheetsLogger sheetsLogger; // Reference to GoogleSheetsLogger
    ...
    private void Start()
    {
        // Get the GoogleSheetsLogger instance in the scene
        sheetsLogger = FindObjectOfType<GoogleSheetsLogger>();
    }
    ...
    public void StopTimer(string expression)
    {
        if (isTiming)
        {
            ...
            Debug.Log("All time points: " + string.Join(", ", timePoints) + " seconds");
            isTiming = false;
            levelStarted = false; // Reset level started for the next level
            
            // Log to Google Sheets if sheetsLogger is available
            if (sheetsLogger != null)
            {
                sheetsLogger.LogLevelData(currentLevel, timeSpent);
                StartCoroutine(dataLogger.LogDataToGoogleSheet(currentLevel, timeSpent, expression));
            }
            else
            {
                Debug.LogWarning("GoogleSheetsLogger not found! Unable to log data to Google Sheets.");
            }
        }
    }
